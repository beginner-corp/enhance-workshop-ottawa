...
function DeleteButtonElement({ html, state }) {
  const { attrs } = state;
  const { key } = attrs;
  return html`
    <style>
...
class DeleteButton extends MorphdomMixin(CustomElement) {
...
function LinkItemElement({ html, state }) {
  const { attrs } = state;
...
class LinkItem extends MorphdomMixin(CustomElement) {
